...
{
  laser_frame_ = scan.header.frame_id;//雷达坐标系名称
  ...
  gsp_laser_beam_count_ = scan.ranges.size();//雷达光速个数

  double angle_center = (scan.angle_min + scan.angle_max)/2;//中心角度
  ...
  for(unsigned int i=0; i<scan.ranges.size(); ++i)//每个光束的角度 ，光束每次增加固定角度
  {
...
SlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose)
{
  ...
  } else 
  {
    ...
    {
      // Must filter out short readings, because the mapper won't
      if(scan.ranges[i] < scan.range_min)     //刷掉很小的值
        ranges_double[i] = (double)scan.range_max;
      else
